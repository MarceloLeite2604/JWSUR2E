...
public class ClientHandlerResolver implements HandlerResolver {
	...
	public List<Handler> getHandlerChain(PortInfo portInfo) {
		...
		/*
		 * ... on an outgoing message, logical handlers execute before message
		 * handlers. Yet even if the order were reversed in this code, the
		 * runtime still would ensure that the logical "IdHandler" executed
	...
	class IdHandler implements LogicalHandler<LogicalMessageContext> {
		...
		public void close(MessageContext lmctx) {
		}
		...
		public boolean handleMessage(LogicalMessageContext lmctx) {
			...
			if (outbound) { // request?
				...
				try {
					JAXBContext jaxbCtx = JAXBContext.newInstance("br.com.acaosistemas.predictionssoapclient");
